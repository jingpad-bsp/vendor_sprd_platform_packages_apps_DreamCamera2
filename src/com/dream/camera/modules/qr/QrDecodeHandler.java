...
final class QrDecodeHandler extends Handler {
    ...
    private void setYuvDataForFreeze(byte[] data, int width, int height) {
        synchronized (this) {
            ...
            Log.d(TAG,"yuv copy cost "+(System.currentTimeMillis()-start));
            mPreviewWidth = width;
    ...
    public Bitmap getPreviewBitmap(Rect previewRect) {
        ...
        if (mYuvData == null || mPreviewWidth == 0 || mPreviewHeight == 0) {
            Log.e(TAG,"mYuvData is "+mYuvData+", mPreviewWidth is "+mPreviewWidth+", mPreviewHeight is "+mPreviewHeight);
            return null;
        }
