...
public class FocusLengthFusionPhotoUI extends PhotoUI implements ResetListener {
    private static final Log.Tag TAG = new Log.Tag("FocusLengthFusionPhotoUI");
    ...
    public FocusLengthFusionPhotoUI(CameraActivity activity, PhotoController controller,
            View parent) {
        ...
        if (DreamUtil.BACK_CAMERA == dreamUtil.getRightCamera(DataModuleManager
            ...
            if (mBasicModule.isBeautyCanBeUsed()) {
                if (topPanel == null) {
                    ...
                    topPanel = lf.inflate(R.layout.fusionphoto_top_makeup_top_panel,
                            topPanelParent);
                }
            } else {
                if (topPanel == null) {
                    LayoutInflater lf = LayoutInflater.from(mActivity);
                    topPanel = lf.inflate(R.layout.fusionphoto_top_panel,
                            topPanelParent);
                }
            }
            ...
            if (v != null && fl != null) {
                if (show == true) {
                    ...
                    int isBeauty = DataModuleManager.getInstance(
                            mActivity.getAndroidContext()).getCurrentDataModule().getInt(Keys.KEY_CAMERA_BEAUTY_ENTERED);
