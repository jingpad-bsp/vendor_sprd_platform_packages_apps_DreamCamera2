...
//                DataConfig.CategoryType.CATEGORY_CAMERA, true, null, 0));
    ...
    public void initializeDataModuleCamera(){
        mDataModuleCamera.initializeData(new DataStructSetting(
                DataConfig.CategoryType.CATEGORY_CAMERA, true, -1, 0));
    }
...
//    }
    ...
    public void addListener(final ResetListener listener) {
        synchronized (mLock) {
            ...
            //SPRD:bug957246 Confirm CameraActivity listener at the first.
            if (listener.getClass().getName().contains("CameraActivity")){
                mSettingResetListeners.add(0,listener);
            } else {
    ...
    public void destory(Context context) {
        ...
        if (mDataModuleCamera != null) {
            mDataModuleCamera.removeAllListeners(context);
        }
        if (mDataModulePhoto != null) {
            mDataModulePhoto.removeAllListeners(context);
        }
        if (mDataModuleVideo != null) {
            mDataModuleVideo.removeAllListeners(context);
        }
...
//        }
        if (mDataModuleTemp != null) {
            mDataModuleTemp.removeAllListeners(context);
        }
    }
}
