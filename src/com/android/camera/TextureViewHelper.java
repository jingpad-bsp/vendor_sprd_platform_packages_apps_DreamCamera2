...
public class TextureViewHelper implements TextureView.SurfaceTextureListener,
    ...
    public TextureViewHelper(TextureView preview, CaptureLayoutHelper helper,
        ...
        mCaptureIntentModeId = ((CameraActivity)appController).getModeIndex();
    }
    ...
    public void updateTransformFullScreen(Matrix matrix, float aspectRatio) {
        ...
        if (Float.compare(aspectRatio,mAspectRatio) != 0) {
            setAspectRatio(aspectRatio);
        }
    ...
    private boolean updateTransform() {
        ...
        CameraId cameraKey = mAppController.getCameraProvider().getCurrentCameraId();
        int cameraId = -1;
        ...
        if (mAppController.getCurrentModuleIndex() == mCaptureIntentModeId) {
            ...
            CameraDeviceInfo.Characteristics info = mAppController.getCameraProvider().getCharacteristics(cameraId);
            // SPRD: fix bug 651778 info object maybe null
            if (info != null) {
    ...
    public Bitmap getPreviewBitmapWithoutTransform(int downsample) {
        RectF textureArea = getTextureArea();
        int width = (int) textureArea.width() / downsample;
        int height = (int) textureArea.height() / downsample;
        Bitmap preview = mPreview.getBitmap(width, height);
        ...
        /* @} */
        return Bitmap.createBitmap(preview, 0, 0, width, height);
    }

    public Bitmap getPreviewBitmapForSwitch(int downsample) {
        RectF textureArea = getTextureArea();
        int width = (int) textureArea.width() / downsample;
        int height = (int) textureArea.height() / downsample;
        Bitmap preview = mPreview.getBitmap(width, height);
    ...
    public void resetPreview() {
        Canvas resetCanvas = new Canvas();
        mPreview.draw(resetCanvas);
    }
    /* @} */
