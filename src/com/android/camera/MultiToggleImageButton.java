...
//        }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, mState, state);
    ...
    private boolean unResponseClick() {
        if (mContext == null){
            return true;
        }
        ...
        if (((CameraActivity) mContext).getCameraAppUI().getFreezeScreenFlag()) {//SPRD:fix bug752287
        ...
        if (((CameraActivity) mContext).getCurrentModule()
            ...
            return true;
                ...
                /* SPRD:fix bug672134 click switch mode and switch front at the same time @{*/
        if (((CameraActivity) mContext).isWaitToChangeMode()) {
            Log.e(TAG, "can not set when camera is switch mode");
            return true;
        }
                /* @} */
        if (((CameraActivity) mContext).getCurrentModule()
                .isTouchCapturing()) {
            Log.e(TAG, "can not set when camera is touchcapturing");
            return true;
        }
                /*during shutter button answer don`t handle switch camera etc*/
        if(((CameraActivity) mContext).getCameraAppUI().isShutterButtonPressed()){
            Log.d(TAG,"shutter button pressed so return directly");
            return true;
        }
        if (!((CameraActivity) mContext).getCameraAppUI().isShutterButtonClickable()) {
            Log.e(TAG, "can not set when shutter button is not clickable");
            return true;
        }
        if (!((CameraActivity) mContext).getCurrentModule().isEnableDetector()) {
            Log.e(TAG, "can not set when all buttons are not clickable");
            return true;
        }
        return false;
    }
