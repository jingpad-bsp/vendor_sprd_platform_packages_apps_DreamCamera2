...
public class RotateImageView extends TwoStateImageView implements Rotatable {
    ...
    // SPRD: Fix bug 538131 enable to scale up bitmap to fill in the screen
    private boolean mScaleupEnabled = false;
    ...
    private boolean checkDrawableAvailable(Drawable drawable){
    ...
    private void refreshCurrentDegree(){
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!checkDrawableAvailable(drawable)) return;

        Rect bounds = drawable.getBounds();
        int w = bounds.right - bounds.left;
        int h = bounds.bottom - bounds.top;

        refreshCurrentDegree();
        ...
        if (mScaleupEnabled) {
            /* SPRD:fix bug543381 SharkL fail to show captured image in full screen while
             * chatting, in landscape mode.
             * @{*/
            if (getScaleType() == ImageView.ScaleType.FIT_CENTER) {
                float ratio;
                if (mCurrentDegree == 270 || mCurrentDegree == 90) {
                    ratio = Math.min((float) width / h, (float) height / w);
                } else {
