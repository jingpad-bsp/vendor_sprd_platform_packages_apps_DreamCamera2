...
/**
 * This class encapsulates the logic of drawing different states of the icon in mode drawer for when
 * it is highlighted (to indicate the current module), or when it is selected by the user. It
 * handles the internal state change like a state list drawable. The advantage over a state list
 * drawable is that in the class multiple states can be rendered using the same drawable with some
 * color modification, whereas a state list drawable would require a different drawable for each
 * state.
 */
/*
 * SPRD Bug:519334 Refactor Rotation UI of Camera. @{ Original Android code: public class
 * ModeIconView extends View {
 */
public class ModeIconView extends View implements Rotatable {
    ...
    private Drawable mIconSelectedDrawable = null;
    private Drawable mIconUnSelectedDrawable = null;
    ...
    public void setIconSelectedDrawable(Drawable drawable) {
        mIconSelectedDrawable = drawable;
    }
    
    public void setIconUnSelectedDrawable(Drawable drawable) {
        mIconUnSelectedDrawable = drawable;
    }

    public Drawable getIconUnSelectedDrawable() {
        return mIconUnSelectedDrawable;
    }

    public Drawable getIconSelectedDrawable() {
        return mIconSelectedDrawable;
    }
    ...
    /**
     * This gets called when the selected state is changed. When selected, the background drawable
     * will use a solid pre-defined color to indicate selection.
     * 
