...
public class BottomBar extends FrameLayout {
    ...
    private static final int CIRCLE_ANIM_DURATION_MS = 200;
    ...
    private static final int MODE_FREEZE_PREVIEW = 4;
    ...
    // SPRD: Fix bug 535110, Photo voice record.
    private ImageView mStopRecordVoiceButton;
...
//    private final Drawable.ConstantState[] mShutterButtonBackgroundConstantStates;
    ...
    public void onFinishInflate() {
        ...
        mCaptureLayout =
                (LinearLayout) findViewById(R.id.bottom_panel_parent);
        mCancelLayout =
                (FrameLayout) findViewById(R.id.bottombar_cancel);
        mCancelLayout.setVisibility(View.GONE);

        // SPRD: Fix bug 535110, Photo voice record.
        mStopRecordVoiceButton = (ImageView) findViewById(R.id.btn_camera_stop_record_voice);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (measureWidth == 0 || measureHeight == 0) {
            /* SPRD:Fix bug 520281
             * call setMeasuredDimension if the measureWidth
             * or measureHeight is 0 to avoid crash.@{ */
            Log.e(TAG, "onMeasure return. [measureWidth = " + measureWidth + "]"
...
//        setupShutterBackgroundForModeIndex(index);
    ...
    public void setShutterButtonEnabled(final boolean enabled) {
            Log.d(TAG,"setShutterButtonEnabled : " + enabled);
    ...
    /* SPRD: Add API For FreezeView Transition */
    public void transitionToFreezeReivewLayout() {
        mCaptureLayout.setVisibility(View.GONE);
        mCancelLayout.setVisibility(View.GONE);
        mIntentReviewLayout.setVisibility(View.GONE);
        mMode = MODE_FREEZE_PREVIEW;
    }

    public boolean isInFreezeReview() {
        return mMode == MODE_FREEZE_PREVIEW;
    }
    ...
    private void checkEveryPartOfBottomPanel(){
        ...
        if (extendPanel == null)
            extendPanel = (LinearLayout) ((Activity) getContext())
                    .findViewById(R.id.extend_panel);
        if (smallFilter == null) {
            smallFilter = (LinearLayout) ((Activity) getContext())
                    .findViewById(R.id.cpu_small_effects_layout_id);
        }
    }
    ...
    protected boolean setFrame(int left, int top, int right, int bottom) {
        ...
        int dpValue = 24;
        navHeight = CameraUtil.getNormalNavigationBarHeight();
        final float scale = getContext().getResources().getDisplayMetrics().scaledDensity;
        slidePanelParentHeight = (int) (dpValue * scale + 0.5f);
        float maspectRation = 0.0f;
        int previeBottom = 0;
        if(mCaptureLayoutHelper != null) {//get preview aspectRation and Rect bottom to calculate extend panel padding
            maspectRation = mCaptureLayoutHelper.getAspectRatio();
            previeBottom = (int) mCaptureLayoutHelper.getPreviewRect().bottom;
        }

        checkEveryPartOfBottomPanel();

        if (slidePanelParent != null)
            slidePanelParentHeight = (int) (dpValue * scale + 0.5f);
        if (CameraUtil.isNavigationEnable()) {
            slidePanelParentHeight = 0;
        }

        if (extendPanel != null) {
            /**
             * bug 687429 ,some special density ,extend panel position will not right
             */
            if(Math.abs(1.34f - maspectRation) < 0.1f && //aspectRation is 4:3 use preview rect bottom to calculate padding
                    pad != 0) { //interval mode no bottom bar and use extend panel to layout saved photo in bottom bar positon
                /**SPRD:Bug785922 the value set by setFrame method in BottomBar.java is unsuitable*/
                int barHeight = getHeight();
                if (barHeight > (bottom - previeBottom + navHeight)) {
                    extendPanel.setPadding(0, 0, 0, 7 * barHeight / 6 + navHeight);
                    mBottomBlackHeight = 7 * barHeight / 6 + navHeight;
                } else {
                    extendPanel.setPadding(0, 0, 0, bottom - previeBottom + navHeight);
                    mBottomBlackHeight = bottom - previeBottom + navHeight;
                }
                /** @}*/
            }else {
                extendPanel.setPadding(0, 0, 0, pad + slidePanelParentHeight + navHeight);
                mBottomBlackHeight = pad + slidePanelParentHeight + navHeight;
            }
        }

        if (smallFilter != null) {
            if (Math.abs(1.34f - maspectRation) < 0.1f &&
                    pad != 0) {
                int barHeight = getHeight();
                if (barHeight > (bottom - previeBottom + navHeight)) {
                    smallFilter.setPadding(0, 0, 0, 7 * barHeight / 6
                            + navHeight);
                } else {
                    smallFilter.setPadding(0, 0, 0, bottom - previeBottom
                            + navHeight);
                }
            } else {
                smallFilter.setPadding(0, 0, 0, pad + slidePanelParentHeight + navHeight);
            }
        }

        frameTop = top - slidePanelParentHeight;

        return super.setFrame(left, top - slidePanelParentHeight, right, bottom
                - slidePanelParentHeight);
    }

    /* SPRD: Fix bug 535110, Photo voice record. @{ */
    public void showStopRecordVoiceButton() {
        mStopRecordVoiceButton.setVisibility(View.VISIBLE);
        mShutterButton.setVisibility(View.INVISIBLE);
    }

    public void hideStopRecordVoiceButton() {
        mStopRecordVoiceButton.setVisibility(View.INVISIBLE);
    ...
    private int mBottomBlackHeight;

    public int getBottomBlackHeight() {
        return mBottomBlackHeight;
    }

    public void setExtendPanelPadding(){
        if (extendPanel == null)
            extendPanel = (LinearLayout) ((Activity) getContext())
                    .findViewById(R.id.extend_panel);
        if (extendPanel != null)
