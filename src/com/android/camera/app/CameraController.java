...
public class CameraController implements CameraAgent.CameraOpenCallback, CameraProvider {
     ...
     * supported.</p>
     */
    private boolean mUsingNewApi = true;
...
//        }
    ...
    public void onCameraOpened(CameraAgent.CameraProxy camera) {
        ...
        */
        if (mRequestingCameraId != camera.getCameraId()
                && mRequestingCameraId < getNumberOfCameras()) {
            mCameraAgentNg.closeCamera(null, true);//SPRD:fix bug936574
            return;
        }
        /* @} */
        ...
        mCameraProxy = camera;
        //mRequestingCameraId = EMPTY_REQUEST;//SPRD:fix bug1346790
    ...
    public void onDisconnected( int cameraId ){
        if (mCallbackReceiver != null)
            mCallbackReceiver.onDisconnected(cameraId);
    }
...
//        }
    ...
    public boolean waitingForCamera() {
        ...
        Log.i(TAG, "waitingForCamera mCameraProxy = " + mCameraProxy);
        return mCameraProxy == null;//SPRD:fix bug1346790
    }
    ...
    public void releaseCamera(int id) {
        ...
        if (currentId != id) {
            if (mRequestingCameraId == id) {
                ...
                Log.w(TAG, "Trying to release a camera neither opened"
    ...
    public void setCameraCloseCallback(CameraAgent.CameraCloseCallback cb) {
        if (mUsingNewApi) {
            mCameraAgentNg.setCameraCloseCallback(cb);
        } else {
            mCameraAgent.setCameraCloseCallback(cb);
        }
    }
    public void removeCameraCloseCallback() {
        if (mUsingNewApi) {
            mCameraAgentNg.removeCameraCloseCallback();
        } else {
            mCameraAgent.removeCameraCloseCallback();
        }
    }
    ...
    public boolean isCameraProxyNull(){
