...
public class MosaicRenderer
{
     static
     {
         try {
             if (CameraUtil.isNewWideAngle()) {
                 System.loadLibrary("jni_mosaic_new");
             } else {
                 System.loadLibrary("jni_mosaic_dream");
             }
         } catch (UnsatisfiedLinkError error) {
             android.util.Log.d("Mosaic", " Load the library dream mosaic is error!!!");
         }
     }
    ...
    public static native void setViewPort(float viewPortX, float viewPortY, int factor, float factor2, float factor3, int orientation, int director);

     public static native void hideSmallView(boolean needHide);

     public static native void setCaptureViewPort(float viewPortX, float viewPortY, int width, int height, boolean land);
}
