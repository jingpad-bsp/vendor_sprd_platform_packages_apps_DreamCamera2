...
/**
 * A view that shows a pop-out effect for a thumbnail image as the new capture
 * indicator design for Haleakala. When a photo is taken, this view will appear
 * in the bottom right corner of the view finder to indicate the capture is
 * done.
 * 
 * Thumbnail cropping: (1) 100% width and vertically centered for portrait. (2)
 * 100% height and horizontally centered for landscape.
 * 
 * General behavior spec: Hide the capture indicator by fading out using
 * fast_out_linear_in (150ms): (1) User open filmstrip. (2) User switch module.
 * (3) User switch front/back camera. (4) User close app.
 * 
 * Visual spec: (1) A 12dp spacing between mode option overlay and thumbnail.
 * (2) A circular mask that excludes the corners of the preview image. (3) A
 * solid white layer that sits on top of the preview and is also masked by 2).
 * (4) The preview thumbnail image. (5) A 'ripple' which is just a white
 * circular stroke.
 * 
 * Animation spec: - For (2) only the scale animates, from 50%(24dp) to
 * 114%(54dp) in 200ms then falls back to 100%(48dp) in 200ms. Both steps use
 * the same easing: fast_out_slow_in. - For (3), change opacity from 50% to 0%
 * over 150ms, easing is exponential. - For (4), doesn't animate. - For (5),
 * starts animating after 100ms, when (1) is at its peak radius and all
 * animations take 200ms, using linear_out_slow in. Opacity goes from 40% to 0%,
 * radius goes from 40dp to 70dp, stroke width goes from 5dp to 1dp.
 */
/*
 * SPRD Bug:527678 Rotate thumnail. @{ Original Android code:
 * 
 * public class RoundedThumbnailView extends View {
 */
public class RoundedThumbnailView extends View implements Rotatable {
    private static final Log.Tag TAG = new Log.Tag("RoundedThumbnailView");

    // Configurations for the thumbnail pop-out effect.
    private static final long THUMBNAIL_STRETCH_DURATION_MS = 40;//SPRD:fix bug851958
    private static final long THUMBNAIL_SHRINK_DURATION_MS = 40;
    ...
    private static final long RIPPLE_DURATION_MS = 40;
    ...
    private View.OnClickListener mOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            final ValueAnimator hitStateAnimator = ValueAnimator.ofFloat(
                    HIT_STATE_CIRCLE_OPACITY_BEGIN,
                    HIT_STATE_CIRCLE_OPACITY_END);
            ...
            hitStateAnimator
                    .addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                        ...
                        public void onAnimationUpdate(
                        	...
                        	Log.i(TAG, "onClick hitStateAnimator2");
                            mCurrentHitStateCircleOpacity = (Float) valueAnimator
                                    .getAnimatedValue();
                            invalidate();
                        }
                    });
    ...
    public RoundedThumbnailView(Context context, AttributeSet attrs) {
        ...
        mThumbnailPadding = getResources().getDimension(
                R.dimen.rounded_thumbnail_padding);
        ...
        mThumbnailStretchDiameterBegin = getResources().getDimension(
                R.dimen.rounded_thumbnail_diameter_min);
        mThumbnailStretchDiameterEnd = getResources().getDimension(
                R.dimen.rounded_thumbnail_diameter_max);
        mThumbnailShrinkDiameterBegin = mThumbnailStretchDiameterEnd;
        mThumbnailShrinkDiameterEnd = getResources().getDimension(
                R.dimen.dream_rounded_thumbnail_diameter_normal);
        ...
        mRippleRingDiameterEnd = getResources().getDimension(
                R.dimen.rounded_thumbnail_ripple_ring_diameter_max);

        mViewRect = new RectF(0, 0, mRippleRingDiameterEnd,
                mRippleRingDiameterEnd);

        mRippleRingDiameterBegin = getResources().getDimension(
                R.dimen.rounded_thumbnail_ripple_ring_diameter_min);
        mRippleRingThicknessBegin = getResources().getDimension(
                R.dimen.rounded_thumbnail_ripple_ring_thick_max);
        mRippleRingThicknessEnd = getResources().getDimension(
                R.dimen.rounded_thumbnail_ripple_ring_thick_min);
        ...
        mBAckgroundCirclePaint.setColor(Color.WHITE);
        mBAckgroundCirclePaint.setStyle(Paint.Style.FILL);

        mDefaultPaint = new Paint();
        mDefaultPaint.setColor(Color.DKGRAY);
        /*
        // SPRD Bug:527678 Rotate thumnail.
        if (context != null && context instanceof CameraActivity)
            ((CameraActivity) getContext()).getOrientationManager()
    ...
    protected void onDraw(Canvas canvas) {
         ...
         * SPRD Bug:527678 Rotate thumnail. @{ Original Android code:
         * 
         * final float centerX = canvas.getWidth() / 2; final float centerY =
         * canvas.getHeight() / 2;
         */
        ...
        if (true) {
            ...
            mBAckgroundCirclePaint.setAlpha((int) (255));
            canvas.drawCircle(centerX, centerY, mRippleRingDiameterEnd / 2
                    + getContext().getResources().getDisplayMetrics().density
                    + 0.5f, mBAckgroundCirclePaint);
            canvas.restore();
        }
        ...
        if (mBackgroundRequest != null) {
            ...
            if (thumbnailPaint != null) {
                ...
                canvas.save();
                canvas.scale(scaleRatio, scaleRatio, centerX, centerY);
                canvas.drawRoundRect(mViewRect, centerX, centerY,
                        thumbnailPaint);
                canvas.restore();
            }
        } else {
        ...
        // Draw animated parts (thumbnail and ripple) if there exists a reveal
        // request.
        if (mForegroundRequest != null) {
            ...
            if (mCurrentRippleRingThickness > 0) {
                ...
                canvas.drawCircle(centerX, centerY,
                        mCurrentRippleRingDiameter / 2, mRipplePaint);
                canvas.restore();
            }

            // Achieve the animation effect by scaling the transformation
            // matrix.
            float scaleRatio = mCurrentThumbnailDiameter
                    / mRippleRingDiameterEnd;
            ...
            // Draw the reveal while circle.
            mRevealCirclePaint
                    .setAlpha((int) (mCurrentRevealCircleOpacity * 255));
            canvas.drawCircle(centerX, centerY, mRippleRingDiameterEnd / 2,
                    mRevealCirclePaint);
        ...
        if (mCurrentHitStateCircleOpacity != HIT_STATE_CIRCLE_OPACITY_HIDDEN) {
            ...
            // Draw the hit state while circle.
            mHitStateCirclePaint
                    .setAlpha((int) (mCurrentHitStateCircleOpacity * 255));
            canvas.drawCircle(centerX, centerY, mRippleRingDiameterEnd / 2,
                    mHitStateCirclePaint);
            canvas.restore();
        }
     ...
     * 
     * @return The diameter of the thumbnail image after the revealing
     *         animation.
     */
     ...
     *            An accessibility String to be announced during the revealing
     *            animation.
     */
    public void startRevealThumbnailAnimation(String accessibilityString) {
        MainThread.checkMainThread();
        // Create a new request.
        mPendingRequest = new RevealRequest(getMeasuredWidth(),
                accessibilityString);
    }
     ...
     * @param rotation
     *            The orientation of the image in degrees.
     */
    ...
    private void runPendingRequestAnimation() {
        ...
        if (ApiHelper.isLOrHigher()) {
            ...
            stretchInterpolator = AnimationUtils.loadInterpolator(getContext(),
                    android.R.interpolator.fast_out_slow_in);
        } else {
            stretchInterpolator = new AccelerateDecelerateInterpolator();
        }

        // The first phase of thumbnail animation. Stretch the thumbnail to the
        // maximal size.
        ...
        stretchAnimator
                .addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator valueAnimator) {
                        mCurrentThumbnailDiameter = (Float) valueAnimator
                                .getAnimatedValue();
                        float fraction = valueAnimator.getAnimatedFraction();
                        float opacityDiff = THUMBNAIL_REVEAL_CIRCLE_OPACITY_END
                                - THUMBNAIL_REVEAL_CIRCLE_OPACITY_BEGIN;
                        mCurrentRevealCircleOpacity = THUMBNAIL_REVEAL_CIRCLE_OPACITY_BEGIN
                                + fraction * opacityDiff;
                        invalidate();
                    }
                });

        // The second phase of thumbnail animation. Shrink the thumbnail to the
        // final size.
        ...
        mThumbnailAnimatorSet.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                Log.i(TAG , "animator set END --- " + animation);
            }

            @Override
            public void onAnimationStart(Animator animation) {
                Log.i(TAG , "animator set START --- " + animation);
            }
        });
        ...
        Interpolator rippleInterpolator = InterpolatorHelper
                .getLinearOutSlowInInterpolator(getContext());

        // When start shrinking the thumbnail, a ripple effect is triggered at
        // the same time.
        mRippleAnimator = ValueAnimator.ofFloat(mRippleRingDiameterBegin,
                mRippleRingDiameterEnd);
        ...
        mRippleAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                Log.i(TAG, "animator END --- " + animation);
            }

            @Override
            public void onAnimationStart(Animator animation) {
                Log.i(TAG , "animator START --- " + animation);
            }
        });
        mRippleAnimator
                .addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator valueAnimator) {
                        mCurrentRippleRingDiameter = (Float) valueAnimator
                                .getAnimatedValue();
                        float fraction = valueAnimator.getAnimatedFraction();
                        mCurrentRippleRingThickness = mRippleRingThicknessBegin
                                + fraction
                                * (mRippleRingThicknessEnd - mRippleRingThicknessBegin);
                        mCurrentRippleRingOpacity = RIPPLE_OPACITY_BEGIN
                                + fraction
                                * (RIPPLE_OPACITY_END - RIPPLE_OPACITY_BEGIN);
                        invalidate();
                    }
                });
    ...
    /**
     * Encapsulates necessary information for a complete thumbnail reveal
     * animation.
     */
    private static class RevealRequest {
        ...
        // The flag to indicate if thumbnail animation of this request is
        // full-filled.
        private boolean mThumbnailAnimationFinished;

        // The flag to indicate if ripple animation of this request is
        // full-filled.
        private boolean mRippleAnimationFinished;

        /**
         * Constructs a reveal request. Use setThumbnailBitmap() to specify a
         * source bitmap for the thumbnail.
         * 
         * @param viewSize
         *            The size of the capture indicator view.
         * @param accessibilityString
         *            The accessibility string of the request.
         */
        ...
        /**
         * Returns the paint object which can be used to draw the thumbnail on a
         * Canvas.
         * 
         * @return the paint object which can be used to draw the thumbnail on a
         *         Canvas.
         */
        ...
        private void precomputeThumbnailPaint(Bitmap srcBitmap, int rotation) {
            ...
            if (mThumbnailPaint == null) {
                // Can't create a paint object until the thumbnail bitmap is
                // available.
                if (srcBitmap == null) {
                    return;
                }
                ...
                if (srcBitmap.getWidth() != mViewSize) {
                    // Create a transformation matrix for the bitmap shader if
                    // the size is not
                    // matched.
                    RectF srcRect = new RectF(0.0f, 0.0f, srcBitmap.getWidth(),
                            srcBitmap.getHeight());
                    ...
                    shaderMatrix.setRectToRect(srcRect, dstRect,
                            Matrix.ScaleToFit.FILL);

                    // Rotate the image around the given source rect point.
                    shaderMatrix.preRotate(rotation, srcRect.width() / 2,
                            srcRect.height() / 2);
         ...
         * 
         * @return True if both thumbnail animation and ripple animation are
         *         finished
         */
        ...
        public void setThumbnailBitmap(Bitmap thumbnailBitmap, int rotation) {
            ...
            if (originalBitmap.getWidth() != originalBitmap.getHeight()) {
                ...
                */
                if (Bitmap.Config.ARGB_8888 != originalBitmap.getConfig()) {
                    originalBitmap = changeBitmap(originalBitmap);
                } else {
                    originalBitmap = cropCenterBitmap(originalBitmap);
                }
            }
        ...
        private Bitmap changeBitmap(Bitmap srcBitmap) {

            int srcWidth = srcBitmap.getWidth();
            int srcHeight = srcBitmap.getHeight();
            int x, y, width, height;
            if (srcWidth >= srcHeight) {
                x = srcWidth / 2 - srcHeight / 2;
                y = 0;
                width = srcHeight;
                height = srcHeight;
            } else {
                x = 0;
                y = srcHeight / 2 - srcWidth / 2;
                width = srcWidth;
                height = srcWidth;
            }
            ...
            // force set config as ARGB_8888.
            bitmap = Bitmap.createBitmap(width, height,
                    Bitmap.Config.ARGB_8888, srcBitmap.hasAlpha());
            bitmap.setHasAlpha(srcBitmap.hasAlpha());
            Canvas canvas = new Canvas(bitmap);
            canvas.translate(-deviceR.left, -deviceR.top);
            canvas.drawBitmap(srcBitmap, srcR, dstR, null);
        ...
        /**
         * Obtains a square bitmap by cropping the center of a bitmap. If the
         * given image is portrait, the cropped image keeps 100% original width
         * and vertically centered to the original image. If the given image is
         * landscape, the cropped image keeps 100% original height and
         * horizontally centered to the original image.
         * 
         * @param srcBitmap
         *            the bitmap image to be cropped in the center.
        ...
        private Bitmap cropCenterBitmap(Bitmap srcBitmap) {
            ...
            Bitmap dstBitmap;
            if (srcWidth >= srcHeight) {
                dstBitmap = Bitmap.createBitmap(srcBitmap, srcWidth / 2
                        - srcHeight / 2, 0, srcHeight, srcHeight);
            } else {
                dstBitmap = Bitmap.createBitmap(srcBitmap, 0, srcHeight / 2
                        - srcWidth / 2, srcWidth, srcWidth);
            }
            return dstBitmap;
        }
    }
