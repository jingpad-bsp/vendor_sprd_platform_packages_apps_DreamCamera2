...
public class FilmstripItemList {
    ...
    public FilmstripItem get(int index) {
        /** SPRD:fix bug610488 IndexOutOfBoundsException
         * Original code
        ...
        try {
            ...
            Log.w(TAG, "Could not get item. Not found: " + index, ex);
            return null;
        }
    }
    ...
    public void removeAll(ArrayList<Uri> collection) {
        for (Uri uri : collection) {
            FilmstripItem item = get(uri);
            mList.remove(item);
            mUriMap.remove(uri);
        }
    }
