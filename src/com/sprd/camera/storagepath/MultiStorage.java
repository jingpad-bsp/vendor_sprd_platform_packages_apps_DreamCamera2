...
public class MultiStorage {
    ...
    public static final String KEY_DEFAULT_OTG = "Otg";
...
//                supportedStorage.add(storageManager.getBestVolumeDescription(vol));
                supportedStorage.add(getStorageKeyString(i + ""));
                Log.d(TAG, "checkSupportedUsbStorage: find storage : " + storageManager.getBestVolumeDescription(vol));
            }
        }
    }

    public static List<String> getUsbStorageDescription() {
        List<String> usbStorageDescription = new ArrayList<>();
        if (mInstance == null) {
            Log.d(TAG, "getUsbStorageDescription: mInstance is null returned!");
            return usbStorageDescription;
        }
        StorageManager storageManager = mInstance.mContext.getSystemService(StorageManager.class);
        List<VolumeInfo> volumes = StorageUtilProxy.getUsbdiskVolumes();
        if(volumes == null){
...
//        for (VolumeInfo vol : volumes) {
        for(int i = 0; i < volumes.size(); i++){
            ...
            File file = vol.getPath();
            if (file == null) {
                Log.d(TAG, "getUsbStorageDescription: storage is wrong");
                continue;
            }
            if (Environment.MEDIA_MOUNTED.equals(StorageUtilProxy.getUsbdiskVolumeState(file))) {
                usbStorageDescription.add(storageManager.getBestVolumeDescription(vol));
                Log.d(TAG, "getUsbStorageDescription: find storage : " + storageManager.getBestVolumeDescription(vol));
            }
        }
        return usbStorageDescription;
    }
    ...
    public static String getDescriptionByKey(String key,
        ...
        if (key == null || descriptionList.size() == 0) {
            return null;
        }

        int i = 0;
        try {
            i = Integer.parseInt(key.substring(KEY_DEFAULT_OTG.length()));
    ...
    static private String getStorageKeyString(String value) {
        return MultiStorage.KEY_DEFAULT_OTG + value;
    }
...
//            StoragePathMap.put(storageManager.getBestVolumeDescription(vol), file.getPath());
            StoragePathMap.put(getStorageKeyString(i + ""), file.getPath());
